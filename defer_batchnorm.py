...
class DeferredBatchNorm(_BatchNorm):
    ...
    def convert_deferred_batch_norm(cls, module: TModule, chunks: int = 1) -> TModule:
        ...
        :class:`nn.BatchNorm`s into :class:`DeferredBatchNorm`::
            ...
            from monepipe.batchnorm import DeferredBatchNorm
            model = resnet101()
    ...
    @classmethod
    def convert_batch_norm_deffered(cls, module: TModule) -> TModule:

        module_output: nn.Module = module

        if isinstance(module, DeferredBatchNorm):
            module_output = _BatchNorm(module.num_features,
                                              module.eps,
                                              module.momentum,
                                              module.affine)
            if module.affine:
        ...
        for name, child in module.named_children():
            module_output.add_module(name, cls.convert_batch_norm_deffered(child))
