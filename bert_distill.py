...
# def ditilbert_to_sequential(model):
#     layers = []
...
#         else:
#             layers.append(child)
#     sequential_layers = torch.nn.Sequential(*layers)

#     #test output
#     test_input = torch.rand((1, 64))
#     with torch.no_grad():
#         x, y = model(test_input)
#         seq_output = sequential_layers.forward(test_input)
...
def bert_to_sequential(model, inputs):
    layers = []
...
def _bert_to_sequential(module, config):
    ...
    for child in module.children():
        if isinstance(child, BertEmbeddings):
            layers.append(BertModelInit(child, config))
...
class BertLastBlock(torch.nn.Module):
    ...
    def forward(self, inputs):
        ...
        if not return_dict:
            output = (prediction_scores,) + bert_outputs[2:]
